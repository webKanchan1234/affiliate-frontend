...
const Category = () => {
  ...
  const {
    ...
    error: productsError
  } = useProductFilters(productsByCategoryAction, (state) => state.productsByCategory);

  // Format category name for display
  const displayCategoryName = useMemo(() => {
    return categoryName.replace(/-/g, ' ').replace(/\b\w/g, char => char.toUpperCase());
  }, [categoryName]);
  ...
  const seoDescription = useMemo(() => {
    ...
    if (count === 0) return `Explore ${displayCategoryName} products at competitive prices`;
    if (count === 1) return `Buy ${displayCategoryName} ${filteredProducts[0].name} at best price`;
    return `Shop ${count}+ ${displayCategoryName} products at best prices with free shipping. Latest models with specifications, reviews & ratings.`;
  }, [displayCategoryName, filteredProducts]);

  // Fetch data on component mount
  useEffect(() => {
    fetchProducts(categoryName);
    dispatch(getAllBrandsAction());
  }, [categoryName, fetchProducts, dispatch]);
      ...
      <div className='flex gap-6'>
        ...
        <main className='w-full lg:w-4/5'>
          <div className="bg-white p-4 rounded-lg shadow-sm mb-4">
            <h1 className='text-2xl md:text-3xl font-bold capitalize'>{displayCategoryName}</h1>
